...
{% partial file="/v1.10/connectors/ingestion-modes-tiles.md" variables={yamlPath: "/connectors/database/timescale/yaml"} /%}
...
When executing stored procedures in TimescaleDB, lineage extraction relies on capturing the SQL queries executed within the procedure. However, by default, PostgreSQL (and therefore TimescaleDB) does not track the internal queries of a stored procedure in `pg_stat_statements`.
...
{% partial
  ...
  variables={
    ...
    selectServicePath: "/images/v1.10/connectors/timescale/select-service.png",
    addNewServicePath: "/images/v1.10/connectors/timescale/add-new-service.png",
    serviceConnectionPath: "/images/v1.10/connectors/timescale/service-connection.png",
}
/%}
...
- **Auth Type**: Basic Auth or IAM based auth to connect to instances / cloud rds.
  - **Basic Auth**:
    - **Password**: Password to connect to TimescaleDB.
  - **IAM Based Auth**:
...
- **Host and Port**: Enter the fully qualified hostname and port number for your TimescaleDB deployment in the Host and Port field.

**SSL Modes**

There are a couple of types of SSL modes that TimescaleDB (PostgreSQL) supports which can be added to ConnectionArguments, they are as follows:
...
{% collateContent %}
{% partial file="/v1.10/connectors/database/timescale/reverse-metadata.md" /%}
{% /collateContent %}
